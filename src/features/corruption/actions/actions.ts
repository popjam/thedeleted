...
import {
  Response,
  ResponseReturnData,
} from "../../../interfaces/corruption/responses/Response";
import { randomInRange } from "../../../types/general/Range";
import { triggerResponse } from "../responses/responses";
...
export function triggerAction(
  ...
  if (action.responses !== undefined) {
    ...
    if (action.tags?.interval !== undefined) {
      let { interval } = action.tags;
      if (typeof interval !== "number") {
        interval = randomInRange(interval);
      }
      if (action.tags.intervalCount === undefined) {
        action.tags.intervalCount = 0;
      }
      action.tags.intervalCount++;
      if (action.tags.intervalCount >= interval) {
        action.tags.intervalCount = 0;
      } else {
        return undefined;
      }
